---
title: How to Localize your Discord Bot in 2026
description: Build a localized Discord bot with li18n, a compile-time i18n library for TypeScript.
---
...
const execute = withLocale(async (interaction: ChatInputCommandInteraction) => {
  ...
  if (minutes < 1) {
    ...
    await interaction.reply({
      content: m.remindTooShort({ minutes }),
      ephemeral: true,
    });
    return;
  }
  ...
  setTimeout(async () => {
    await interaction.followUp(
      m.remindFire({ username: interaction.user.username, message }),
    );
  }, minutes * 60_000);
}, resolveLocale);
...
<Callout type='info'>
  If you got a lot of commands with the same locale resolver, you can create a
  helper class that wraps `withLocale` and hardcodes the resolver, so you don't
...
<Callout type='info'>
  With watch mode, it is recommended to change your save behavior in your editor
  to "save on focus change" or increase the debounce delay, to avoid triggering
  a build on every keystroke (this could result in bugs).
</Callout>
