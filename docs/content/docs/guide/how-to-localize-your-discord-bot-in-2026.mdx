...
<Callout type="info">
  If you got a lot of commands with the same locale resolver, you can create a helper class that
  wraps `withLocale` and hardcodes the resolver, so you don't have to repeat it everywhere.
</Callout>
...
<Callout type="info">
  With watch mode, it is recommended to change your save behavior in your editor to "save on focus
  change" or increase the debounce delay, to avoid triggering a build on every keystroke (this could
  result in bugs).
</Callout>
